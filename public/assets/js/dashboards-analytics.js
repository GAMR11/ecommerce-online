...
(function () {
  ...
  // --------------------------------------------------------------------
    const categories = JSON.parse(document.getElementById('topCategorias').value);
    const valorCategorias = JSON.parse(document.getElementById('valorCategorias').value);
    console.log('categories:',categories);
    console.log('valorCategorias:',valorCategorias);
    // console.log('categories:',categories);
  const chartOrderStatistics = document.querySelector('#orderStatisticsChart'),
    orderChartConfig = {
      ...
      labels: valorCategorias.map(item=>item.categoria),
      // labels: ['Electronic', 'Sports', 'Decor', 'Fashion'],
      series: valorCategorias.map(item=>parseInt(item.porcentaje)),
  ...
  // --------------------------------------------------------------------
  const pagosPorMes = JSON.parse(document.getElementById('pagosPorMes').value);

  // Extraemos categorías (meses) y valores
  const categoriesPagos = pagosPorMes.map(pago => pago.mes);
  const valores = pagosPorMes.map(pago => pago.total_pagado);
  ...
  const incomeChartConfig = {
      ...
      discrete: [
        {
          ...
          seriesIndex: 0,
          dataPointIndex: valores.length - 1, // Último punto destacado
    ...
    xaxis: {
      categories: categoriesPagos, // 🔥 Se pasan los nombres dinámicamente
    ...
    yaxis: {
      labels: { show: false },
      min: Math.min(...valores) - 10, // Ajustamos el mínimo dinámicamente
      max: Math.max(...valores) + 10, // Ajustamos el máximo dinámicamente
      tickAmount: 4
    }
  };
